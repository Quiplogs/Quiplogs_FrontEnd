...
import { ScheduleWeekly, ScheduleWeeklyAdapter } from '../models/schedule-weekly';
...
export class ScheduleWeeklyService {

    private apiType: string = 'ScheduleWeekly';

    constructor(
        private http: HttpClient,
        private apiEndpointService: ApiEndpointsService,
        private scheduleWeeklyAdapter: ScheduleWeeklyAdapter,
    ...
    public get(id: string): Observable<ScheduleWeekly> {
        const headers = new HttpHeaders()
            .set('Content-Type', 'application/json');
        ...
        return this.http.get(url, { headers }).pipe(
            map((data: any) => this.scheduleWeeklyAdapter.adapt(data)),
            catchError(err => {
                this.apiMessageHandlerService.handleError(err);
                return of(null);
            }),
    ...
    public delete(id: string, showToast: boolean) {
        ...
        return this.http.delete(url, { headers }).pipe(
            map((data: any) => {
                if (showToast) {
                    this.apiMessageHandlerService.handleSuccess('Succesfully deleted Weekly Schedule');
                }
            }),
            catchError(err => {
                this.apiMessageHandlerService.handleError(err);
    ...
    public put(modelEntity: ScheduleWeekly): Observable<ScheduleWeekly> {
        const headers = new HttpHeaders()
            .set('Content-Type', 'application/json');
        ...
        return this.http.put(this.apiEndpointService.apiUrl(this.apiType), JSON.stringify(model), { headers }).pipe(
            map((data: any) => this.scheduleWeeklyAdapter.adapt(data), this.apiMessageHandlerService.handleSuccess('Succesfully updated Weekly Schedule')),
            catchError(err => {
                this.apiMessageHandlerService.handleError(err);
                return of(null);
            }),
